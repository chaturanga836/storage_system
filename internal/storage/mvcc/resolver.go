...
// HashBytesToString creates a hash string from bytes
func HashBytesToString(data []byte) string {
...
// VersionedRecord represents a record with MVCC information
...
// Resolver is an alias for VersionResolver to match storage_manager.go expectations
type Resolver = VersionResolver

// NewResolver creates a new MVCC resolver
func NewResolver(config *MVCCConfig) *Resolver {
	return NewVersionResolver()
}

// NewVersion generates a new version number
func (vr *Resolver) NewVersion() uint64 {
	return vr.GetNextVersion()
}

// MergeResults merges results from memtable and persistent storage
func (vr *Resolver) MergeResults(memtableResults, persistentResults []*VersionedRecord) []*VersionedRecord {
	// Create a map to store the latest version of each record
	resultMap := make(map[string]*VersionedRecord)

	// Process memtable results first (they are newer)
	for _, record := range memtableResults {
		if !record.Deleted {
			resultMap[record.ID] = record
		}
	}

	// Process persistent results
	for _, record := range persistentResults {
		if existing, exists := resultMap[record.ID]; !exists || record.Version > existing.Version {
			if !record.Deleted {
				resultMap[record.ID] = record
			}
		}
...
// MVCCConfig configuration (placeholder until we import it)
